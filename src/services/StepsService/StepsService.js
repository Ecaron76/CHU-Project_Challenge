...
export const StepsService = {
    ...
    getSteps: async function(pkId) {
        ...
        var mondayFiveWeeksBefore = new Date(now.getTime() - daysToSubtract * 24 * 60 * 60 * 1000).toJSON().substring(0,10); // Soustraire les jours en millisecondes
        ...
        const arrayOfMonth = await this.createArrayForMonthData(fiveMonthStepsData, fiveMonthAgo, todayDate);
    ...
    createArrayForMonthData: async function(fiveMonthStepsData, oldDate, nowDate) {
    ...
    createArrayForWeeksData: async function(fiveMonthStepsData, oldDate, nowDate) {
        ...
        const fiveWeeksArray = fiveMonthStepsData.filter((result) => {
            ...
            const isTrue = resultDate >= mondayFiveWeeksBefore && resultDate <= today ? true : false;
        ...
        const arrayOfDates = await this.buildArrayOfDates(mondayFiveWeeksBefore, today);

        // On itère sur chaque object du tableau arrayOfDates ex => {"monday": 2024-01-29T00:00:00.000Z, "sunday": 2024-02-04T00:00:00.000Z}
        arrayOfDates.forEach((dateObject)=>{ // Marchouille ! régler le problème du dernier lundi ...
    ...
    buildArrayOfDates: async (mondayFiveWeeksBefore, today) => {
        ...
        while (mondayFiveWeeksBefore <= today) {
            ...
            if ((isMonday || isSunday) && flag < 2 ) {
                ...
                isMonday ? object["monday"] = new Date(currentDate) : object["sunday"] = new Date(currentDate) ;
    ...
    createArrayForDaysData: async function(fiveMonthStepsData, oldDate, nowDate) {
        ...
        while (currentDate <= today) {
            ...
            if(stepsArray.length === 0) {
                dailyStepsArray.push({count: 0, date: currentDate.toJSON().substring(0,10)});
            }
            else { // Sinon on push la valeur dans stepsArray
