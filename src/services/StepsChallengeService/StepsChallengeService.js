...
getWeeksStepsStoredData: async function (challengeId) {
  ...
  let dateOfFirstDay = new Date(new Date(dateOfLastSunday).setDate(dateOfLastSunday.getDate()-27));
...
doCallsAndMakeArray: async function (challengeId) {
  ...
  const stepsDataForOtherMonths = await this.getMonthStepsStoredData(challengeId);

  const stepsDataForOtherWeeks = await this.getWeeksStepsStoredData(challengeId);

  const stepsDataForOtherDays = await this.getDaysStepsStoredData(challengeId);
  ...
  arrayOfSteps.push(stepsDataForOtherMonths, stepsDataForOtherWeeks, stepsDataForOtherDays);
